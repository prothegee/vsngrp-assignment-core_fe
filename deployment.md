...
Core FE holds no secrets and mounts no config file at runtime, `VITE_API_BASE_URL` and `VITE_WS_BASE_URL` are baked into the static bundle at Docker build time instead, see `adr.md`.
...
The production API and WS origins come from `CORE_FE_ENV_API_BASE_URL` and `CORE_FE_ENV_WS_BASE_URL`, not literal values in `cd.yml`. Since Vite bakes `VITE_*` variables into the bundle at build time, a wrong value here can only be fixed by correcting the secret and redeploying, never by editing a file on the running server.
